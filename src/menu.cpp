 ...
 * Last Modified : Sat 18 Aug 2012 02:40:07 PM EEST
...
{
    tinyFont = GLUT_BITMAP_HELVETICA_18;
    nextY = -DIMENSION/4.;
    hs.get_scores(hscores);
}
void scorescreen::display()
{
    ...
    std::string text;
    drawMenu(0);
    for(unsigned int i=0; i<hscores.size(); i++)
    {
        text = *hscores[i];
        glRasterPos2f(-DIMENSION/4.,DIMENSION/4.-i/5.);
